...
const LOG_COPIA_GRAFICO = "[IRoboPF][copiar-imagen-grafico]"
const ESCALA_MAXIMA_EXPORTACION_GRAFICO = 2
const FAMILIA_MONO_GRAFICO_EXPORTADO =
...
function BotonCopiarImagenGrafico({
  graficoRef,
  disabled,
}: {
  ...
  const copiarImagen = React.useCallback(async () => {
    ...
    try {
      console.info(LOG_COPIA_GRAFICO, "inicio", {
        clipboardWriteDisponible: Boolean(navigator.clipboard?.write),
        clipboardItemDisponible: typeof ClipboardItem !== "undefined",
        contextoSeguro: window.isSecureContext,
      })
      await copiarElementoComoImagenAlPortapapeles(grafico)
      fijarEstado("copiada")
      console.info(LOG_COPIA_GRAFICO, "exito")
    } catch (error) {
      console.error(LOG_COPIA_GRAFICO, "error", error)
      fijarEstado("error")
    } finally {
  ...
  const ariaLabel = Match.value(estado).pipe(
    Match.when("copiada", () => "Imagen de la gráfica copiada"),
    Match.when("error", () => "No se pudo copiar la imagen de la gráfica"),
    Match.orElse(() => "Copiar imagen de la gráfica")
  )
    ...
    >
      ...
      <span aria-live="polite" className="sr-only">
        {Match.value(estado).pipe(
          Match.when("copiada", () => "Imagen copiada al portapapeles."),
          Match.when("error", () => "No se pudo copiar la imagen."),
          Match.orElse(() => "")
        )}
      </span>
    </Button>
...
async function copiarElementoComoImagenAlPortapapeles(elemento: HTMLElement) {
  console.info(LOG_COPIA_GRAFICO, "generando png")
  const blob = await convertirElementoEnPng(elemento)
  console.info(LOG_COPIA_GRAFICO, "png generado", {
    bytes: blob.size,
    tipo: blob.type,
  })
  ...
  if (!navigator.clipboard?.write || typeof ClipboardItem === "undefined") {
    console.info(
      LOG_COPIA_GRAFICO,
      "navigator.clipboard.write no disponible; usando fallback execCommand"
    )
  ...
  try {
    console.info(LOG_COPIA_GRAFICO, "copiando con navigator.clipboard.write")
    ...
    console.error(
      LOG_COPIA_GRAFICO,
      "navigator.clipboard.write fallo; probando fallback execCommand",
      error
    )
  }
...
async function copiarBlobPngConExecCommand(blob: Blob) {
  ...
  try {
    ...
    seleccion?.removeAllRanges()
    console.info(LOG_COPIA_GRAFICO, "resultado fallback execCommand", {
      copiado,
    })
    if (!copiado) {
...
async function convertirElementoEnPng(elemento: HTMLElement): Promise<Blob> {
  ...
  const url = crearUrlSvgDesdeElemento({ elemento, ancho, alto })

  try {
    console.info(LOG_COPIA_GRAFICO, "cargando svg serializado como imagen", {
      ancho,
      alto,
    })
    ...
    dibujarMarcaAguaGrafico({ contexto, elemento, ancho, alto })
    console.info(LOG_COPIA_GRAFICO, "canvas renderizado", {
...
function crearUrlSvgDesdeElemento({
  ...
  forzarFuenteGraficoSvg(graficoClonado)
  ...
  const leyenda = crearSvgLeyendaGrafico({ elemento, rectContenedor })
  const svg = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${ancho}" height="${alto}" viewBox="0 0 ${ancho} ${alto}">`,
    crearEstilosSvgGraficoExportado(),
    `<rect x="0" y="0" width="100%" height="100%" fill="${fondo}" />`,
    contenido,
    leyenda,
    "</svg>",
  ].join("")
  console.info(LOG_COPIA_GRAFICO, "svg serializado", {
    caracteres: svg.length,
    incluyeLeyenda: leyenda.length > 0,
  })

  return URL.createObjectURL(new Blob([svg], { type: MIME_SVG }))
}

function crearEstilosSvgGraficoExportado() {
  const reglasFuente = obtenerReglasFuenteMonoExportacion()
  return `<style><![CDATA[
${reglasFuente.join("\n")}
svg, text, tspan {
  font-family: ${FAMILIA_MONO_GRAFICO_EXPORTADO};
}
]]></style>`
}

function obtenerReglasFuenteMonoExportacion() {
  const reglas: string[] = []
  for (const hoja of Array.from(document.styleSheets)) {
    let reglasCss: CSSRuleList
    try {
      reglasCss = hoja.cssRules
    } catch {
      continue
    }

    for (const regla of Array.from(reglasCss)) {
      if (!(regla instanceof CSSFontFaceRule)) continue
      if (!regla.cssText.includes("JetBrains Mono")) continue
      reglas.push(reescribirUrlsCssRelativas(regla.cssText, hoja.href))
    }
  }
...
function reescribirUrlsCssRelativas(css: string, hrefHoja: string | null) {
  if (!hrefHoja) return css

  return css.replace(/url\((["']?)([^"')]+)\1\)/g, (_, comilla, url) => {
    const urlAbsoluta = new URL(url, hrefHoja).toString()
    return `url(${comilla}${urlAbsoluta}${comilla})`
  })
}

function forzarFuenteGraficoSvg(grafico: SVGSVGElement) {
  grafico.style.fontFamily = FAMILIA_MONO_GRAFICO_EXPORTADO
  for (const texto of Array.from(
    grafico.querySelectorAll<SVGTextElement>("text")
  )) {
    texto.setAttribute("font-family", FAMILIA_MONO_GRAFICO_EXPORTADO)
    texto.style.fontFamily = FAMILIA_MONO_GRAFICO_EXPORTADO
  }
}
...
function obtenerSvgPrincipalGrafico(elemento: HTMLElement) {
  ...
  console.info(LOG_COPIA_GRAFICO, "svg principal seleccionado", {
    superficies: candidatas.length,
    ancho: principal?.ancho ?? 0,
    alto: principal?.alto ?? 0,
    area: principal?.area ?? 0,
  })

  return principal?.superficie ?? null
}

function crearSvgLeyendaGrafico({
  elemento,
  rectContenedor,
}: {
  readonly elemento: HTMLElement
  ...
  const estilosLeyenda = window.getComputedStyle(leyenda)
  const fontFamily = escaparAtributoSvg(FAMILIA_MONO_GRAFICO_EXPORTADO)
  const fontSize = parseFloat(estilosLeyenda.fontSize) || 14
  const fontWeight = escaparAtributoSvg(estilosLeyenda.fontWeight || "700")
  const nodos = items
    .map((item) => {
      ...
      const estilosTrazo = elementoTrazo
        ...
        : estilosTexto
      const colorTexto = escaparAtributoSvg(estilosTexto.color)
      const colorTrazo = escaparAtributoSvg(
      ...
      return [
        `<line x1="${numeroSvg(xIcono)}" y1="${numeroSvg(y)}" x2="${numeroSvg(xIcono + 14)}" y2="${numeroSvg(y)}" stroke="${colorTrazo}" stroke-width="3" stroke-linecap="square" />`,
        `<text x="${numeroSvg(xTexto)}" y="${numeroSvg(y)}" fill="${colorTexto}">${escaparTextoSvg(texto)}</text>`,
      ].join("")
    })
    .join("")

  return `<g font-family="${fontFamily}" font-size="${numeroSvg(fontSize)}" font-weight="${fontWeight}" dominant-baseline="central">${nodos}</g>`
}
...
function dibujarMarcaAguaGrafico({
  ...
  contexto.save()
  contexto.font = `700 14px ${FAMILIA_MONO_GRAFICO_EXPORTADO}`
...
function obtenerYMarcaAguaGrafico({
  ...
  const yPorDefecto = alto - margen
  const rectContenedor = elemento.getBoundingClientRect()
  const etiquetasEjeX = Array.from(
    elemento.querySelectorAll<SVGTextElement>(
      ".recharts-xAxis .recharts-cartesian-axis-tick text"
    )
  )
  const limiteSuperiorEjeX = etiquetasEjeX.reduce<number | null>(
    (limite, etiqueta) => {
      const rect = etiqueta.getBoundingClientRect()
      if (rect.width === 0 && rect.height === 0) return limite

      const superior = rect.top - rectContenedor.top
      return limite === null ? superior : Math.min(limite, superior)
    },
    null
  )
  if (limiteSuperiorEjeX !== null) {
    return Math.max(
      margen + altoCaja,
      Math.min(yPorDefecto, limiteSuperiorEjeX - margen)
    )
  }
  ...
  if (!leyenda) return yPorDefecto

  const rectLeyenda = leyenda.getBoundingClientRect()
  const marcaIzquierda = ancho - margen - anchoCaja
  const marcaDerecha = ancho - margen
  const leyendaIzquierda = rectLeyenda.left - rectContenedor.left
  const leyendaDerecha = rectLeyenda.right - rectContenedor.left
  const solapaHorizontalmente =
    marcaIzquierda < leyendaDerecha && marcaDerecha > leyendaIzquierda
  if (!solapaHorizontalmente) return yPorDefecto

  const yEncimaLeyenda = rectLeyenda.top - rectContenedor.top - margen
  return Math.max(margen + altoCaja, Math.min(yPorDefecto, yEncimaLeyenda))
}
